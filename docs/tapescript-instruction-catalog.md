...
| keep | 动作 | `Fire(Forward)` / `Fire()` | 向正前方的敌对目标开火。 | 如果正前方没有敌对目标，动作失败，自动播放应暂停；如果命中，会削减目标 HP，并在击毁时清空该格。 | `Fire()` |
...
| keep | 查询 | `Check(Forward).Has(Enemy)` / `Check().Has(Enemy)` | 检查正前方一格是否存在敌对目标。 | 设置条件标志为 true 或 false；在没有真实敌人配置的旧原型关卡里，会继续兼容确定性占位信号。 | `Check().Has(Enemy)` |
